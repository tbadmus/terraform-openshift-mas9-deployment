...
module "vpc" {
  ...
  availability_zones = local.aws_azs
  aws_other_subnets  = var.aws_other_subnets
...
module "installer" {
  ...
  workspace_region                  = var.workspace_regions[terraform.workspace]
  terraform_workspace               = terraform.workspace
  tags                              = local.tags
}

# Post-deployment cleanup.. kubeadmin-password and kubeconfig are stored in Secrets Manager
# resource "null_resource" "post_deploy_cleanup" {
#   provisioner "local-exec" {
#     command = "rm -rf ${path.root}/installer-files " #${path.root}/kubeadmin-password ${path.root}/kubeconfig
#   }
